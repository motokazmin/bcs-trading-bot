...
Два канала выгрузки, они НЕ взаимозаменяемы:

| что нужно | чем забирать |
|-----------|--------------|
| посмотреть глазами / отдать ИИ | кнопка «Скачать incident.json» на `/export` |
| `make analyze`, `metrics.csv`, водяной знак | онлайн-бэкап `trades.db` (скрипт читает SQLite, не JSON) |

### 1. Забрать данные с хоста

```bash
# БД — для make analyze. Только .backup: scp одного файла без -wal
# отдаёт состояние на последний чекпоинт.

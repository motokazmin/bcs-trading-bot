...
// Strategy — самодостаточная торговая стратегия (Фаза 3, ADR 0001,
// docs/adr/0001-engine-strategy-boundary.md). В отличие от CandleStrategy
// (только сигнал на вход, всё остальное решает TickerWorker), Strategy сама
// владеет своим торговым циклом: подписывается на данные через
// StrategyContext, сама решает сайзинг/выход/трейлинг/частичные закрытия.
// Единственное, что она обязана делать через каркас — запрашивать риск
// (RiskPort) и сохранять закрытые сделки (TradeRecorder).
//

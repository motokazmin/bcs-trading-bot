...
// для всех воркеров/стратегий в процессе. Согласно ADR 0001
// (docs/adr/0001-engine-strategy-boundary.md), DataFeed — часть каркаса:
...
// Feed собирает подписки от воркеров/стратегий до старта и затем прогоняет
// единую WebSocket-сессию, раздавая свечи/тики в зарегистрированные каналы.
//
// Использование:
//  1. Subscribe(...) для каждого воркера — до вызова Run.
...
// разными воркерами — каждый получит свою копию потока (fan-out на уровне
// BCSClient, не на уровне Feed). tickIn может быть nil, если воркеру тики не
// нужны (например, чисто свечная стратегия без внутрибарового SL/TP).
//

...
func TestGenerateWindows(t *testing.T) {
	...
	windows := core.GenerateWindows(start, end, 2, 1)
	if len(windows) == 0 {
...
func TestScoreRejectsLowTrades(t *testing.T) {
	m := core.Metrics{NumTrades: 5, Calmar: 10}
	if s := core.Score(m, 20); !math.IsInf(s, -1) {
		t.Fatalf("expected -Inf, got %v", s)
	}
}

func TestScoreCalmar(t *testing.T) {
	m := core.Metrics{NumTrades: 30, Calmar: 2.5, MaxDrawdown: 1.0, TotalPnL: 2.5}
	if s := core.Score(m, 20); s != 2.5 {
		t.Fatalf("score: got %v, want 2.5", s)
	}
}

func TestScoreLosingUsesPnL(t *testing.T) {
	m := core.Metrics{NumTrades: 30, Calmar: -1, MaxDrawdown: 50000, TotalPnL: -41179.48}
	if s := core.Score(m, 20); s != -41179.48 {
		t.Fatalf("score: got %v, want TotalPnL", s)
	}
}

func TestMedianFloatIgnoresInf(t *testing.T) {
	got := core.MedianFloat([]float64{math.Inf(-1), -100, -200, -300})
	if got != -200 {
		t.Fatalf("median: got %v, want -200", got)
	}
	if !math.IsInf(core.MedianFloat([]float64{math.Inf(-1)}), -1) {
		t.Fatal("empty after filter should be -Inf")
	}
}

func TestNetPnLFromGross(t *testing.T) {
	net := core.NetPnLFromGross(100, 10, 5.0)
	if net != 50 {
...
func TestComputeMetricsDrawdown(t *testing.T) {
	m := core.ComputeMetrics([]float64{100, -50, -30}, []float64{1, -0.5, -0.3})
	if m.TotalPnL != 20 {

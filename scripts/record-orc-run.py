#!/usr/bin/env python3
"""Записывает результат ORC-прогона в results/orc/runs-registry.json и печатает сравнение."""
...
def upsert_registry(registry_path: Path, entry: dict) -> list[dict]:
    data = {"strategy": "opening_range_continuation", "runs": []}
...
def main() -> None:
    p = argparse.ArgumentParser(description="Record ORC optimizer run to registry")
    p.add_argument("--run-id", required=True)
    p.add_argument("--output-dir", required=True)
    p.add_argument("--search-space", required=True)
    p.add_argument("--tickers", default="configs/shared/tickers-orc.yaml")
    p.add_argument("--trials", type=int, default=200)
    p.add_argument("--seed", type=int, default=1)
    p.add_argument("--description", default="")
    p.add_argument(
        "--registry",
        default="results/orc/runs-registry.json",
        help="path to runs registry JSON",
    )
    args = p.parse_args()
    ...
    entry = collect_run(
        ...
        trials=args.trials,
        seed=args.seed,
        description=args.description or f"ORC run {args.run_id}",
    )
    runs = upsert_registry(Path(args.registry), entry)
